...
fetch_t::fetch_t(
	...
	actions["fetch_fetch"].add_trigger([&](){
		return !bp_port.peek() && insn_fetch_event_t.size() && !occupied;
	});
	...
	actions["fetch_decode"].add_trigger([&](){
		return mem_retire_port.size() && occupied;
	});
}