...
public:
	...
	int64_t sb_imm() const {
		return (x(8, 4) << 1) + (x(25, 6) << 5) + (x(7, 1) << 11) +
			   (imm_sign() << 12);
	}
	int64_t u_imm() const { return int64_t(b) >> 12 << 12; }
	int64_t uj_imm() const {
		return (x(21, 10) << 1) + (x(20, 1) << 11) + (x(12, 8) << 12) +
			   (imm_sign() << 20);
	}
	...
	uint64_t fr() const { return x(25, 2); }
	...
	int64_t rvc_addi4spn_imm() const {
		return (x(6, 1) << 2) + (x(5, 1) << 3) + (x(11, 2) << 4) +
			   (x(7, 4) << 6);
	}
	int64_t rvc_addi16sp_imm() const {
		return (x(6, 1) << 4) + (x(2, 1) << 5) + (x(5, 1) << 6) +
			   (x(3, 2) << 7) + (xs(12, 1) << 9);
	}
	int64_t rvc_lwsp_imm() const {
		return (x(4, 3) << 2) + (x(12, 1) << 5) + (x(2, 2) << 6);
	}
	int64_t rvc_ldsp_imm() const {
		return (x(5, 2) << 3) + (x(12, 1) << 5) + (x(2, 3) << 6);
	}
	int64_t rvc_swsp_imm() const { return (x(9, 4) << 2) + (x(7, 2) << 6); }
	int64_t rvc_sdsp_imm() const { return (x(10, 3) << 3) + (x(7, 3) << 6); }
	int64_t rvc_lw_imm() {
		return (x(6, 1) << 2) + (x(10, 3) << 3) + (x(5, 1) << 6);
	}
	int64_t rvc_ld_imm() const { return (x(10, 3) << 3) + (x(5, 2) << 6); }
	int64_t rvc_j_imm() const {
		return (x(3, 3) << 1) + (x(11, 1) << 4) + (x(2, 1) << 5) +
			   (x(7, 1) << 6) + (x(6, 1) << 7) + (x(9, 2) << 8) +
			   (x(8, 1) << 10) + (xs(12, 1) << 11);
	}
	int64_t rvc_b_imm() const {
		return (x(3, 2) << 1) + (x(10, 2) << 3) + (x(2, 1) << 5) +
			   (x(5, 2) << 6) + (xs(12, 1) << 8);
	}
...
private:
	...
	uint64_t xs(int lo, int len) const {
...
#define VS3P(pos) READP_VREG(insn.rd(), pos)
...
#define dirty_fp_state                                                         \
	(STATE.mstatus |= MSTATUS_FS | (xlen == 64 ? MSTATUS64_SD : MSTATUS32_SD))
#define dirty_ext_state                                                        \
	(STATE.mstatus |= MSTATUS_XS | (xlen == 64 ? MSTATUS64_SD : MSTATUS32_SD))
...
#define get_field(reg, mask)                                                   \
	(((reg) & (decltype(reg))(mask)) / ((mask) & ~((mask) << 1)))
#define set_field(reg, mask, val)                                              \
	(((reg) & ~(decltype(reg))(mask)) |                                        \
	 (((decltype(reg))(val) * ((mask) & ~((mask) << 1))) &                     \
	  (decltype(reg))(mask)))
...
#define set_pc_and_serialize(x)                                                \
	do {                                                                       \
		reg_t __npc = (x)&p->pc_alignment_mask();                              \
...
inline freg_t fsgnj128(freg_t a, freg_t b, bool n, bool x) {
	a.v[1] = (a.v[1] & ~F64_SIGN) |
			 (((x ? a.v[1] : n ? F64_SIGN : 0) ^ b.v[1]) & F64_SIGN);
	return a;
}
