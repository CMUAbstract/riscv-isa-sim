...
std::string tinyConsole::version() { return TINYCON_VERSION; }

void tinyConsole::pause() { getch(); }

void tinyConsole::quit() { _quit = true; }
...
int tinyConsole::hotkeys(char c) { return 0; }

void tinyConsole::setMaxHistory(int max) { _max_history = max; }

std::string tinyConsole::getLine() { return getLine(M_LINE, ""); }

std::string tinyConsole::getLine(int mode) { return getLine(mode, ""); }

std::string tinyConsole::getLine(int mode = M_LINE,
...
void tinyConsole::run() {
	...
	for (;;) {
		...
		if (!hotkeys(c))
			switch (c) {
			case ESC:
				// TODO escape is only detected if double-pressed. this should
				// be fixed
				std::cout << "(Esc)";
				break;
			case KEY_CTRL1: // look for arrow keys
				...
				case UP_ARROW:
					...
					// store in buffer
					for (unsigned int i = 0; i < history[pos].size(); i++) {
				...
				case DEL:
					if (line_pos < buffer.size()) {
						...
						for (unsigned int i = line_pos; i < buffer.size();
							 i++) {
							std::cout << buffer[i];
						}
						// erase last char
			...
			case BACKSPACE:
				...
				if (line_pos == buffer.size()) {
					...
					buffer.erase(buffer.begin() + line_pos);
					// update screen after current position
					for (unsigned int i = line_pos; i < buffer.size(); i++) {
						std::cout << buffer[i];
					}
					// erase last char
					std::cout << " ";
					for (unsigned int i = line_pos + 1; i < buffer.size();
						 i++) {
						std::cout << "\b";
					}
